                            ...
                            <tr>
                                ...
                                <td>
                                    @foreach ($factures as $facture)
                                        {{-- @if ($clients->id == $facture->client_id) --}}

                                            <form>
                                                @csrf
                                                {{-- {{user->name}} --}}
                                                {{-- ligne fonctionnelle --}}
                                                {{ $factures->name }}<a href="{{ route('facture.download', ['facture' => $factures->client_id])}}" class="btn"><img src="{{ asset('images/bootstrap.svg')}}" style="background-color:green;" with="10" alt="download"/></a>
                                                {{--  --}}
